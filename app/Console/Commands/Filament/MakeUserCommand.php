...
namespace App\Console\Commands\Filament;
...
class MakeUserCommand extends Command
{
    ...
    protected function getUserData(): array
    {
        return [
            ...
            'username' => $this->validateInput(fn () => $this->ask('username'), 'username', ['required', 'unique:' . $this->getUserModel()]),
    ...
    protected function sendSuccessMessage(Authenticatable $user): void
    {
        ...
        $this->info('Success! ' . ($user->getAttribute('username') ?? $user->getAttribute('username') ?? 'You') . " may now log in at {$loginUrl}.");
